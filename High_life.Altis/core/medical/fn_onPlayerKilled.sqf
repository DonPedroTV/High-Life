...
/*_unit spawn {
    private ["_maxTime2","_Timer2","_zyje"];
    disableSerialization;
    _Timer2 = ((findDisplay 7300) displayCtrl 7309);
    _maxTime2 = time + 600;
    _zyje = _unit getVariable "Revive";
    waitUntil {_Timer2 ctrlSetText format [localize "STR_Medic_Respawn3",[(_maxTime2 - time),"MM:SS"] call BIS_fnc_secondsToString];
    round(_maxTime - time) <= 0 || isNull _this || isNil "_zyje"};
    if (isNil "_zyje") exitWith {};
    closeDialog 0;
    life_respawned = true;
